...
- `model.display_name`
- `effort.level` — the session's reasoning-effort level, resolved by Claude Code itself
...
- **Reasoning-effort badge** on line 2 (`⚙ <level>`) reads `effort.level` straight off the stdin payload — Claude Code (≥2.1.x) resolves the session's effort itself and hands the answer over, so this is the only source that tracks `/effort` and `--effort` live. Do **not** go back to re-deriving it: `/effort` no longer persists `effortLevel` into `settings.json`, so any settings-derived value is a stale leftover that freezes the badge on whatever was last written there. The old chain (`$CLAUDE_CODE_EFFORT_LEVEL` env var, with the sentinels `unset` / `auto` meaning "defer to settings", then the `effortLevel` key in `settings.local.json` / `settings.json` / `~/.claude/settings.json`) is kept only as a fallback for older CLI builds that omit the field. The list of valid level names is discovered dynamically by parsing `claude --help` and cached at `/tmp/claudebar-effort-levels.cache`; the cache is refreshed when it's empty or when the resolved `claude` binary is newer, so new CLI versions that add levels pick up automatically. That parse must join the flag line with the two following lines: the help wraps, and the `(low, medium, high, xhigh, max)` list lands on the *next* row. If the CLI isn't on `PATH`, the script falls back to the known-good list `low / medium / high / xhigh / max`. Colors are assigned by ordinal position within the detected list using a 5-bucket gradient (green → cyan → yellow → bright-red → red), so adding or removing levels upstream won't require tweaking the palette. Values not in the list render gray to signal "unrecognized".
...
The only automated test is `tests/effort-badge.sh`, which locks down the effort badge's resolution order (run it with `bash tests/effort-badge.sh`; it takes an optional path so you can point it at an installed copy). Everything else is smoke-tested by piping a sample JSON payload through the script:
